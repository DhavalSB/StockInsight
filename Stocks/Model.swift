...
func getSector(stock: String) -> String {
    ...
    do {
        ...
        if try (try doc.select("p[class='D(ib) Va(t)'] > span").indices.contains(0)) && (try doc.select("p[class='D(ib) Va(t)'] > span")[0].text() == "Sector(s)") {
            print(try doc.select("p[class='D(ib) Va(t)'] > span")[1].text())
            let retrievedSector = try doc.select("p[class='D(ib) Va(t)'] > span")[1]
        ...
        } else if try (try doc.select("span[class='Mend(5px) Whs(nw)'] > span").indices.contains(5)) && (try doc.select("span[class='Mend(5px) Whs(nw)'] > span")[5].text() == "Legal Type"){
            let type = try doc.select("span[class='Fl(end)']")[4]
